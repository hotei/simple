...
// simple example of svg use by ajstark
// see ajstark's svg package for more details
// usage ./svgAndroid > android.svg  then view the result in a browser
// or svg capable file viewer - like Nautilus
...
func android(x, y int, fill string, opacity float64) {
	...
	canvas.Circle(x+100, y+45, 5, `fill="red"`)                                     // right eye
...
func main() {
	...
	canvas.Scale(3.0)
	android(50, 50, "gray", 1.0)
	canvas.Gend()

	canvas.Scale(0.5)
	android(100, 100, "red", 0.5)
	canvas.Gend()
	canvas.End()
}
